...
function parseVueComponent(parts, options) {
    ...
    var hasScope = parts.styles.some(function (item) {
    ...
    if (tplOpts.compileToRender === false
        && options.isProduction && !tpl.error
    ) {
        tpl.output.code = processor.minifyHtml(tpl.output.code, tplOpts.minify);
    }
 ...
 * @param {string} options.insertCssLibDir 插入 css lib 所在的目录，用于确定 insertCSSPath
 * @param {string} options.hotReloadApiDir hot reload api 所在目录，用于确定 hotReloadAPIPath
 ...
 * @param {Object|boolean=} options.template.transformEle 要转换处理的元素，可选，
 *        默认 img.src,source.srcset，如果不转换传 false
 ...
 * @param {Function=} options.template.preprocessor 自定义的模板的预处理器，可选
 ...
 *        等价于{postcss: {plugins: []}}
 * @param {Array} options.style.postcss.plugins postcss 使用的插件，只支持同步的
 * @param {Object} options.style.postcss.options postcss 的选项
 * @param {boolean|Function=} options.style.urlRewrite 是否进行 url 重写，或者自定义的重写方法，可选
