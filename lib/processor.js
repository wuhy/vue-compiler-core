...
function processVuePart(part, filePath, opts, needSourceMap) {
    ...
    var lang = part.lang || opts.lang;
    if (!lang && source.src) {
        var type = path.extname(source.src).replace(/^\./, '');
        lang = require('./file-type')[type];
    }
...
exports.processStyle = function (part, filePath, id, opts, needSourceMap) {
    ...
    var data = rewriteStyle(
        ...
        code, part.scoped, opts, output.map
    );
    if (data.error) {
        result.error = data.error;
    }
    else {
        data.deps = data.deps.concat(output.deps);
        result.output = data;
    }
